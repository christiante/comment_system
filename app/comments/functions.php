<?php
	...
	session_start();
	$_SESSION['user'] = 'Christiane';



	function get_total() {
		require 'connect.php';
		$result 	= mysqli_query($connect, "SELECT * FROM `parent_comments` ORDER BY `date` DESC");
		$row_cnt 	= mysqli_num_rows($result);
		echo '<div class="comments_total">All Comments ('.$row_cnt.')</div>';
	}
	...
	function get_comments() {
		...
		foreach($result as $item) {
			...
			$chi_result = mysqli_query($connect, "SELECT * FROM `child_comments` WHERE `par_code`='$par_code' ORDER BY `date` DESC");
			$chi_cnt = mysqli_num_rows($chi_result);

			if($chi_cnt == 0){
			} else {
				echo '<a class="link-reply" id="children" name="'.$par_code.'">'
					.'<span id="tog_text">replies</span> ('.$chi_cnt.')</a>'
